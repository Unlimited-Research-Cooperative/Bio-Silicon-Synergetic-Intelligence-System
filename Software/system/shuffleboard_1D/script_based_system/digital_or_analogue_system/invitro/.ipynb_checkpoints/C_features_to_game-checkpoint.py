...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QTableWidget, QTableWidgetItem, QMessageBox
...
class FeaturesToGameAction(QWidget):
    def __init__(self, mqtt_client, selected_channels=None):
        ...
        self.pause = False  # Flag to control the pause in sending packets
    ...
    def handle_outcome(self, outcome):
        if outcome in ["reward", "distress"]:
            self.pause = True
            Timer(1, self.end_pause).start()  # Pause for 1 second using threading.Timer
...
def on_message(client, userdata, message):
    try:
        ...
        if topic == "EXTRACTED FEATURES":
            ...
            outcome = json.loads(payload).get("outcome")
            userdata.handle_outcome(outcome)
