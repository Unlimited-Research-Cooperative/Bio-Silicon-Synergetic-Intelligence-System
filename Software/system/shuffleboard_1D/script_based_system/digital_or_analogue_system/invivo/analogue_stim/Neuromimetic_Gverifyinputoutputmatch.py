...
input_signal_log = []
output_signal_log = [[] for _ in range(NUM_CHANNELS)]
lock = threading.Lock()
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        with lock:
            ...
            timestamp = time.time()
            input_signal_log.append((timestamp, latest_waveform.copy()))
...
def audio_callback_factory(channel):
    def audio_callback(in_data, frame_count, time_info, status):
        global ring_buffers, ring_buffer_write_positions, output_signal_log, lock
        ...
        # Log the output signal for verification
        timestamp = time.time()
        output_signal_log[channel].append((timestamp, chunk.copy()))
...
def main():
    ...
    try:
        ...
        compare_signals(input_signal_log, output_signal_log)

def compare_signals(input_log, output_log):
    # Define the latency for comparison
    latency = 0.1  # Adjust the latency based on your system's processing delay
    # Define the scaling factor for the output signals
    SCALING_FACTOR = 4000 * 32767
    
    for i in range(NUM_CHANNELS):
        for input_time, input_signal in input_log:
            # Locate the output signals that correspond to the input signals in time
            output_times = [time for time, signal in output_log[i] if time >= input_time + latency]
            if output_times:
                closest_output_time = min(output_times, key=lambda t: abs(t - (input_time + latency)))
                output_signal = [signal for time, signal in output_log[i] if time == closest_output_time][0]
                input_signal_scaled = input_signal[i] * SCALING_FACTOR  # Apply scaling to input signal

                print(f"Channel {i} - Input at {input_time}: {input_signal[:5]}")
                print(f"Channel {i} - Scaled Input: {input_signal_scaled[:5]}")
                print(f"Channel {i} - Output at {closest_output_time}: {output_signal[:5]}")

                # Correct the latency if needed
                corrected_latency = closest_output_time - input_time
                print(f"Channel {i} - Corrected Latency: {corrected_latency}")
