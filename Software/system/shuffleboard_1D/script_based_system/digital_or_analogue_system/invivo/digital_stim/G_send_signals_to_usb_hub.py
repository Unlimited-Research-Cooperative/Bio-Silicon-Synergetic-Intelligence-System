    ...
    def update_plot(self):
        ...
        if latest_waveform is not None:
            for i in range(self.num_channels):
                ...
                # Send the data to PyAudio devices
                self.send_to_audio_device(i, self.data[i])

    def send_to_audio_device(self, channel_index, data):
        global ring_buffers, ring_buffer_write_positions, lock
        with lock:
            write_pos = ring_buffer_write_positions[channel_index]
            to_write = data
            available_space = RING_BUFFER_SIZE - write_pos
            if len(to_write) > available_space:
                ring_buffers[channel_index][write_pos:] = to_write[:available_space]
                ring_buffers[channel_index][:len(to_write) - available_space] = to_write[available_space:]
            else:
                ring_buffers[channel_index][write_pos:write_pos + len(to_write)] = to_write
            ring_buffer_write_positions[channel_index] = (write_pos + len(to_write)) % RING_BUFFER_SIZE
            print(f"Updated ring buffer for channel {channel_index} with first 5 data points: {ring_buffers[channel_index][:5]}")
