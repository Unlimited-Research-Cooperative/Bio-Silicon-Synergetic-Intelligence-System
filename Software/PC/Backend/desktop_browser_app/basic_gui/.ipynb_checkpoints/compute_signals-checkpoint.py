...
from util import create_features_dict, get_features, get_all_vals
...
def scale_signals_to_bit_depth(modified_signals, bit_depth_):
...
def apply_phase_synchronization(modified_signals, global_sync_level, pairwise_sync_level, sync_factor=0.05):
    ...
    for i in range(num_signals):
        ...
        global_phase_shift = sync_factor * np.interp(common_phase, (common_phase.min(), common_phase.max()),
                                                     (-np.pi, np.pi))
...
def apply_transfer_entropy(modified_signals, influence_factor):
    ...
    for i in range(1, max_length):
        ...
        influenced_signals = np.dot(interaction_weights,
                                    [signal[i - 1] if i < len(signal) else signal[-1] for signal in modified_signals])
...
def apply_normalized_states(modified_signals, modification_factor, matrix_size, value_range, num_matrices,
                            eigenvalue_subset):
    num_signals, signal_length = modified_signals.shape
    ...
    for i, signal in enumerate(modified_signals):
        ...
        signal_modification = np.interp(signal, (np.min(signal), np.max(signal)),
                                        (np.min(density_diagonal), np.max(density_diagonal)))
...
def final_modified_scale_signals_to_bit_depth(modified_signals):
...
def create_transformation() -> list:
    # Define the list of transformation functions using features
    transformations = [
        lambda x: scale_signals_to_bit_depth(x, 16),
        ...
        lambda x: apply_moving_average(x, int(features["window_size"])),
        ...
        lambda x: apply_phase_synchronization(x, features["global_sync_level"], features["pairwise_sync_level"],
                                              features["sync_factor"]),
        ...
        lambda x: apply_multivariate_empirical_mode_decomposition(x, int(features["num_imfs"])),
        lambda x: apply_normalized_states(x, modification_factor=0.1, matrix_size=10, value_range=(-1, 1),
                                          num_matrices=5,
                                          eigenvalue_subset=slice(0, 5)),
    ]
    return transformations


# Main function to generate and transform signals
def generate_transformed_signals(signal_length, num_signals, transformations, fs, duration, min_volt, max_volt):
    # Now fs and duration are passed in dynamically, ensuring these are defined
    length = fs * duration  # Calculate the length based on current fs and duration
    time_interval = 1 / fs  # Calculate time interval based on current fs
    modified_signals = generate_ecog_like_base_signals(length, num_signals, min_volt, max_volt)

    for transform in transformations:
        modified_signals = transform(modified_signals, min_volt, max_volt) 
    ...
    modified_signals = final_modified_scale_signals_to_bit_depth(modified_signals)
    print(f"Scaled range: {modified_signals.min()} to {modified_signals.max()}")
    # Generate and transform the ECoG-like signals
    transformations = create_transformation()  # Ensure this is called after parameters are defined
