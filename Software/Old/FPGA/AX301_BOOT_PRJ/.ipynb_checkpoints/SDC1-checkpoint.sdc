...
# Constraint for BCLK (Bit Clock)
create_generated_clock -name bclk -source [get_ports {clk}] -divide_by <CLOCK_DIVISOR_BCLK> [get_pins {<i2s_tx_instance_name>/bclk}]
...
# Constraint for MCLK (Master Clock)
create_generated_clock -name mclk -source [get_ports {clk}] -divide_by <CLOCK_DIVISOR_MCLK> [get_pins {<i2s_tx_instance_name>/mclk}]
